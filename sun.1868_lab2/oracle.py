...
def paddingOracle(key, enc):
    padedPlaintext = decrypt(key, enc)
    pad = True
    length = len(padedPlaintext) - len(unpad(padedPlaintext))

    paded = padedPlaintext[-ord(padedPlaintext[len(padedPlaintext)-1:]):]
    for x in paded:
        if ord(x) == length and length<=16:
            continue
        else:
            pad = False
    return pad
...
if __name__ == '__main__':
    ...
    if '-e' in myargs:
        ...
        pad = paddingOracle(key, ciphertext)
        if pad:
